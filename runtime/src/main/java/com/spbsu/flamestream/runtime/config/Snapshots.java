...
public class Snapshots<Element> {
  ...
  private static final int verticesNumber =
          System.getenv().containsKey("ACKER_VERTICES_NUMBER")
                  ? parseInt(System.getenv("ACKER_VERTICES_NUMBER")) : 100;
  ...
  private static final int window =
          System.getenv().containsKey("SNAPSHOTS_WINDOW") ? parseInt(System.getenv("SNAPSHOTS_WINDOW")) : 10;
  ...
  private void checkPeriodIncrement(long time, Consumer<Supplier<Stream<Element>>> scheduleSnapshotDone) {
    if (period(time) > scheduledPeriod) {
      System.out.println("SNAPSHOT " + scheduledPeriod);
      scheduledPeriod++;
