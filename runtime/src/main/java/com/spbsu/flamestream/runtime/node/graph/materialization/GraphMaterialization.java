...
public class GraphMaterialization extends LoggingActor {
  private final Map<String, Stream<VertexMaterialization>> materialization = new HashMap<>();

  private GraphMaterialization(Graph graph, ActorRef router, ActorRef acker, ActorRef barrier) {

  }

  public static Props props(Graph graph, ActorRef router, ActorRef acker, ActorRef barrier) {
  ...
  private void onAddressedItem(AddressedItem atomicMessage) {
    final Stream<VertexMaterialization> nextVertices = materialization.get(atomicMessage.vertexId());
