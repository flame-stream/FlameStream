...
public class TfIdfGraph implements Supplier<Graph> {
  ...
  public Graph get() {
    ...
    final FlameMap<TextDocument, TfIdfObject> splitterTF = new FlameMap<>(new Function<TextDocument, Stream<TfIdfObject>>() {
      ...
      @Override
      public Stream<TfIdfObject> apply(TextDocument s) {
        TfIdfObject tfIdfObject = new TfIdfObject(s.name(), pattern.split(s.content()), s.partitioning(), counter.incrementAndGet());
    ...
    final FlameMap<TfIdfObject, TfIdfObject> filterClassifier = new FlameMap<>(
            classifier,
            TfIdfObject.class,
            new Runnable() {
              @Override
