...
public class LentaTest extends FlameAkkaSuite {
    private Stream<TextDocument> documents() throws IOException {
        CSVParser csvFileParser = CSVParser.parse(
                new File("/home/tyoma/Work/TVC/media_space/resources/news_lenta.csv"), Charset.defaultCharset(), CSVFormat.DEFAULT
        );
        ...
        return records.map(r -> {
            ...
            return new TextDocument(r.get(4), text.substring(1), String.valueOf(ThreadLocalRandom.current().nextInt(0, 10)));
        }).limit(600000);
    }

    @Test
    public void lentaTest() throws InterruptedException, IOException {
        int nExpected = 600000
                ;
