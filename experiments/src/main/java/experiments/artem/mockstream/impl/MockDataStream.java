...
import experiments.artem.mockstream.DataStream;
...
class MockDataStream implements DataStream {
  private Stream<IntDataItem> _stream;
  private Set<Condition> _violated;

  static MockDataStream generate(Supplier<IntDataItem> s, Set<Condition> violated) {
    return new MockDataStream(s, violated);
  }

  private MockDataStream(Supplier<IntDataItem> s, Set<Condition> violated) {
    _stream = Stream.generate(s);
    _violated = violated;
  }
  ...
  public boolean isValid() {
    for (Condition condition : _violated) {
      if (_stream.anyMatch(s -> !condition.update(s))) {
        return false;
      }
    }
    return true;
