...
public class WorkerApplication {
  public static void main(final String... args) {
    try {
      new WorkerApplication().run(new InetSocketAddress(InetAddress.getLoopbackAddress(), 1212));
    } catch (IOException e) {
  ...
  private void run(final InetSocketAddress address) throws IOException {
    ...
    final ActorRef concierge = system.actorOf(NodeConcierge.props(address, zooKeeper), "concierge");
  }
}
